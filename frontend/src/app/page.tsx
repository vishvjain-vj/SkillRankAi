...
export default function Home() {
  ...
  // ── Login ─────────────────────────────────────────────────────────────────
  // ── Backend Sync (Replaces manual handleLogin) ──────────────────────────
  useEffect(() => {
    async function syncWithBackend() {
      // If Google says they are logged in, and we have their email...
      if (status === "authenticated" && session?.user?.email) {
        try {
          // We use their secure Google Email as their unique ID!
      ...
      // If they log out, clear the data
    ...
    syncWithBackend();
  }, [status, session]); // This runs automatically anytime their auth status changes
  ...
  // ── Login screen ──────────────────────────────────────────────────────────
  if (!user) {
    // Show a sleek loading state while NextAuth connects to Google
    if (status === "loading") {
      return (
    ...
    return (
      <div className="min-h-screen relative flex items-center justify-center p-4">
        ...
        {/* 1. HEADER SECTION (Anchored to the top of the screen) */}
        <div className="absolute top-[5%] left-0 right-0 mx-auto w-full max-w-xl text-center px-4">
          ...
          <div className="relative w-[150px] h-[150px] mx-auto mb-6 flex justify-center bg-black rounded-full overflow-hidden border border-white/5">
            <Image
              src="/logo.png" 
              alt="Platform Logo" // Points to image_10.png renamed to logo.png in public/
              fill
              sizes="200px"
              priority
              /* object-contain ensures it fits perfectly without distortion */
